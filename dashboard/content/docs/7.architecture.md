---
title: Architecture
icon: i-lucide-network
navigation:
  icon: i-lucide-network
---

# Service Architecture

AriLink consists of several interconnected services that work together to handle voice calls with AI.

## Components

```mermaid
graph TB
  subgraph PBX["Asterisk PBX"]
    SIP[SIP Trunks]
    DP[Dialplan]
    ARI_EP[ARI Endpoint]
  end

  subgraph AriLink["AriLink Server"]
    API[REST API]
    SIO[Socket.IO]
    CTRL[ARI Controller]
    AF[Assistant Factory]
    AD[Auto Dialer]
  end

  subgraph Audio["Audio Pipeline"]
    RUST[Rust RTP Server]
    TR[Transcription Service]
  end

  subgraph UI["Dashboard"]
    WEB[Vue 3 SPA]
  end

  SIP --> DP
  DP --> ARI_EP
  ARI_EP -- WebSocket --- CTRL
  CTRL --> AF
  CTRL --> AD
  CTRL -- ExternalMedia --> RUST
  RUST -- WebSocket PCM --> TR
  TR -- Text --> CTRL
  API --- WEB
  SIO --- WEB
```

## Service Details

| Service | Technology | Purpose |
|---------|-----------|---------|
| **Nuxt/Nitro Server** | Node.js + TypeScript | Dashboard UI, REST API, Socket.IO, call orchestration |
| **ARI Connection** | ari-client (WebSocket) | Call control — answering, bridging, transferring, playing audio |
| **Rust RTP Server** | Rust binary | Receives raw RTP audio, converts to PCM, streams to transcription |
| **Transcription** | Parakeet TDT / Google Speech | Real-time speech-to-text via WebSocket |
| **TTS** | Kokoro (local) / ElevenLabs (cloud) | Text-to-speech synthesis for assistants and OpenClaw |
| **Asterisk PBX** | FreePBX | SIP trunk management, extensions, audio routing |

## Data Flow

1. Caller dials in → **Asterisk** routes to ARI Stasis app
2. **ARI** notifies Node.js → assistant is selected (routing rules or default)
3. Audio is bridged to **Rust RTP server** → PCM stream to **transcription**
4. Transcription text flows back → assistant decides action (play audio, transfer, collect data)
5. When the assistant speaks back → **TTS** synthesizes audio → played on the call channel
6. Dashboard shows real-time status via **Socket.IO**

## Ports & Connections

| Service | Default Port | Protocol |
|---------|-------------|----------|
| Dashboard (Nuxt) | 3011 | HTTP/WS |
| Asterisk ARI | 8088 | HTTP/WS |
| Rust RTP | 9100 (control) | HTTP |
| Rust RTP | 10000-20000 (media) | UDP/RTP |
| Transcription (Parakeet) | 5000 | WebSocket |
| TTS (Kokoro) | 5001 | WebSocket |

## Container Management

When running in Docker, the dashboard can restart individual containers and display their status (state, health, uptime). This uses the Docker socket mounted into the arilink container.

- **Restart**: Available via dashboard buttons or `POST /api/docker/restart`
- **Status**: Polled every 15 seconds, available via `GET /api/docker/status`
- **Managed containers**: `arilink-parakeet`, `arilink-kokoro`, `arilink-asterisk`, `arilink-rust-rtp`

When Docker is not available (local development), container management is hidden and only software-level reconnect buttons are shown.

## Transcription Fallback

```mermaid
flowchart LR
  A[Audio Stream] --> B{Primary service up?}
  B -- Yes --> C[Parakeet TDT]
  B -- No --> D{Fallback configured?}
  D -- Yes --> E[Google Cloud Speech]
  D -- No --> F[Error - no transcription]
```

Configure via `TRANSCRIPTION_SERVICES` env variable:
- `ws://localhost:5000` — Parakeet only
- `ws://localhost:5000,google` — Parakeet primary, Google fallback
- `ws://localhost:5000,ws://localhost:5001` — Two Parakeet instances
